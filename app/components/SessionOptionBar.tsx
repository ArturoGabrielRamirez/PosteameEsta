...
export default function SessionOptionBar() {
    ...
    return (
        <PaperBG>
            ...
            <div className="absolute rounded-md p-1 md:p-2 lg:p-4 xl:p-6 flex flex-col max-[639px]:flex-col sm:flex-col gap-1 md:gap-2 sm:items-stretch xl:m-4 sm:max-w-[250px] md:max-w-[280px]">
                <div className="flex gap-1 border-2 sm:border-4 rounded-md sm:gap-2 justify-center sm:justify-between items-center bg-gradient-to-b border-orange-950 from-[#ffe501] via-[#fcc101] to-[#b29400] md:max-h-[100px]">
                    ...
                    <h2 className="hidden sm:block font-bold max-w-[130px] sm:text-lg xl:text-xl">Bienvenido {session?.user?.name}</h2>
                </div>
                <div className="flex gap-1 max-[639px]:flex-row sm:gap-2 xl:justify-between">
                    <SigninButton />
                    <ModeToggle />
                </div>
            </div>
